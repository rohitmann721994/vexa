---
name: k6-performance
description: Modern load testing with k6 including thresholds, scenarios, and custom metrics. Use when writing, reviewing, or debugging a k6 script — general k6 syntax and patterns, not Vexa-specific scaffolding.
---
...
> **Using this in Vexa:** this skill is vendored verbatim as a general k6
> reference. For how performance tests are wired into *this* repo — the
> `k6/` module layout, reusing `ApiLibrary`'s captured cookie instead of
> logging in, and the HTML+JSON report convention — use the
> `write-performance-test` skill and
> [docs/performance-testing-guide.md](../../../docs/performance-testing-guide.md)
> instead. Come back here for generic k6 syntax (executors, custom metrics,
> data-driven tests, etc.).
...
> Note for Vexa: this app is an SPA behind SSO with no server-side login
> form, so k6 cannot `setup()` its own login — use `k6/utils/auth.js`
> (reuses the cookie `ApiLibrary` already captured) instead of this pattern.
